// =====================================================================
// VITS Text Encoder - Part 5: Multi-pattern GPU Parallelism (修正版)
// Advanced Computer Architecture Final Project
//
// 本版修正 (同 Part 4):
//   [1] 初始化順序對齊: 先權重後 x
//       -> pattern 0 的輸入與 Part 4 / CPU 版相同, pattern 0 的輸出貢獻
//          應接近 Part 1 的 99.2277, 可作為正確性驗證錨點
//   [2] linear_proj 改用「靜態」shared memory (__shared__ float s_in[FILTER_DIM])
//       -> ptxas -v 直接顯示 smem 用量
//
// 硬性規定:
//   - 多 pattern 併行, 2D Grid: X 軸 = 原 thread 配置, Y 軸 = blockIdx.y = pattern
//   - ncu 分析 pattern 數對 occupancy / GPU 使用率 / latency hiding 的影響
//
// 編譯:
//   nvcc -O2 -arch=sm_75 -Xptxas -v text_encoder_part5.cu -o text_encoder_part5
//
...
__global__ void linear_proj_kernel(const float* __restrict__ in,
    ...
    __shared__ float s_in[FILTER_DIM];        // 靜態 shared memory (開最大 in_dim)
...
int main() {
    ...
    // --- Host 初始化 ---
    //  ★ 順序對齊 Part 4 / CPU: 先權重後 x
    //    weight 消耗相同的 rand 序列段 -> pattern 0 的 x 與 Part 4 相同
    //    -> pattern 0 的輸出貢獻可對照 Part 1 的 99.2277
    ...
    init_random(h_x);     // x 最後 (NUM_PATTERNS 份輸入)
    ...
    // pattern 0 的 checksum (應接近 Part 1 的 99.2277, 驗證錨點)
    ...
    printf("Pattern-0 Checksum: %f  (應接近 Part 1 的 99.2277)\n", checksum_p0);
