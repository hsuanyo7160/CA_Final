// =====================================================================
// VITS Text Encoder - Part 4: CUDA SIMT (main.cu)
// Advanced Computer Architecture Final Project
//
// SIMT: 1 thread = 1 output task ; linear_proj 用靜態 shared memory
// 測資: 確定性公式 (與 main_cpu.cpp 完全相同) -> GPU sum 可直接比對 CPU sum
//
// 編譯:
//   nvcc -O2 -arch=sm_75 -Xptxas -v main.cu -o main
//   (sm_75 換成你 GPU 的 compute capability)
//
// PTX:  nvcc -arch=sm_75 -ptx main.cu -o main.ptx
// ncu:  ncu --set full ./main
// =====================================================================
...
#define BLOCK_SIZE  256    // block size 旋鈕 (Part 4: 掃描 occupancy)
...
// =====================================================================
// Kernel: Linear Projection (靜態 shared memory)
// =====================================================================
...
__global__ void qkt_kernel(const float* __restrict__ Q,
    ...
    if (idx >= NUM_HEADS * SEQ_LEN * SEQ_LEN) return;
...
// =====================================================================
// Kernel: Layer Normalization (1 thread = 1 token)
// =====================================================================
